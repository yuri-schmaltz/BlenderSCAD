...
def is_bsgroup(o):
    ...
    if o is None:
        return False
    return (
        o.get("blenderscad_group", False)
        and o.draw_type == "BOUNDS"
        and len(o.children) > 0
        and len(o.data.vertices) == 8
        and len(o.data.edges) == 12
    )
...
def color(rgba=(1.0, 1.0, 1.0, 0), o=None):
    ...
    if (
        o.draw_type == "WIRE" or o.draw_type == "BOUNDS"
    ):  # 'WIRE' seems to cause probe /w material...
        return o
...
def rotate(a=[0.0, 0.0, 0.0], v=[0, 0, 0], *args):
    ...
    for arg in args:  # look for object in args...
        if type(arg) == bpy_types.Object:
            o = arg
    if (
        o is None and type(v) == bpy_types.Object
    ):  # dirty hack.. object slipped unnamed into v
        o = v
    ...
    z = o.location[2]
    for i in range(0, 2):
        o.location[i] = round(o.location[i], 2)
    #     y-Transform
...
def group(o1, *objs):
    ...
    )  # bb ~ reference to this bounding box representing group
    ...
    for obj in objs:
        ...
        if is_bsgroup(obj):
            obj.hide = True
            # nested groups to be hidden. only to pertain hierarchy.
        # Keep Hierarchy selectable, but avoid transforming children independent of parent.
        obj.lock_location = (True, True, True)
        obj.lock_rotation = (True, True, True)
        obj.lock_scale = (True, True, True)
...
def apply2objects(objs, o_func, nested=True):
    for o in objs:
        if o.type == "MESH":
            ...
            if is_bsgroup(
                o
            ):  # recurse FIRST (e.g. some stuff like delete may require this order...)
...
def cloneOLD(objs):
    ...
    )  # after cloning, all new objects are selected...
    new_top_objs = []
    # distill all objects without parent in same set...
    for n in new_objs:
        if n.parent not in new_objs:
            new_top_objs.append(n)
...
def remove_duplicates():
    for obj in bpy.context.selected_objects:
        if obj.type == "MESH":
            bpy.data.scenes[0].objects.active = (
                obj  # make obj active to do operations on it
            )
            bpy.ops.object.mode_set(
                mode="OBJECT", toggle=False
            )  # set 3D View to Object Mode (probably redundant)
            bpy.ops.object.mode_set(
                mode="EDIT", toggle=False
            )  # set 3D View to Edit Mode
            bpy.context.tool_settings.mesh_select_mode = [
                False,
                False,
                True,
            ]  # set to face select in 3D View Editor
            bpy.ops.mesh.select_all(
                action="SELECT"
            )  # make sure all faces in mesh are selected
            bpy.ops.object.mode_set(
                mode="OBJECT", toggle=False
            )  # very silly, you have to be in object mode to select faces!!

            found = set([])  # set of found sorted vertices pairs

            for face in obj.data.polygons:
                facevertsorted = sorted(
                    face.vertices[:]
                )  # sort vertices of the face to compare later
                if (
                    str(facevertsorted) not in found
                ):  # if sorted vertices are not in the set
                    found.add(str(facevertsorted))  # add them in the set
                    obj.data.polygons[face.index].select = (
                        False  # deselect faces i want to keep
                    )
            ...
            bpy.ops.mesh.remove_doubles(
                threshold=0.0001, use_unselected=False
            )  # remove doubles
            bpy.ops.mesh.normals_make_consistent(
                inside=False
            )  # recalculate normals (this one or two lines above is redundant)
            bpy.ops.object.mode_set(
                mode="OBJECT", toggle=False
            )  # set to Object Mode AGAIN
    return obj
...
def booleanOp(objA, objB, boolOp="DIFFERENCE", apply=True):
    ...
    #
    if is_bsgroup(objA):
        objA = objA.children[0]
        # first object represents the group.
    if is_bsgroup(objB):
        objB = objB.children[0]
        # first object represents the group.
...
def difference2(o1, *objs, apply=True):
    res = o1
    cleanup_object(
        res, removeDoubles=False, quads=False, subdivide=False, normalsRecalcOut=False
    )
    tmp = res.name
    to = None
    for obj in objs:
...
def projection(o=None, cut=False):
    ...
    if cut:
        ...
        for i in range(len(o.data.polygons) - 1, -1, -1):  # for p in o.data.polygons:
            ...
            if (
                p.normal[2] <= 0.0
            ):  # dirty hack: all faces pointing "down" will be removed
...
def rotate_extrude(o=None, fn=None, fs=None, fa=None):
    ...
    bpy.context.area.type = (
        "VIEW_3D"  # probably: need to set cursor for Spin to be right...
    )
    # print(o.location)
    ...
    bb = o.bound_box[4][
        0
    ]  # boundbox 8x<x,y,z>->( LoX,LoY,LoZ, LoX,LoY,HiZ, LoX,HiY,HiZ, LoX,HiY,LoZ, HiX,LoY,LoZ, HiX,LoY,HiZ, HiX,HiY,HiZ, HiX,HiY,LoZ ).
    r = (
        o.location[0] + bb
    )  # outer radius of object.. X-location defines inner "hole", plus bound box outer in X direction..
    # print("boundBox"); print(bb); print("radius"); print(r);
    segments = blenderscad.core.get_fragments_from_r(r=r, fn=fn, fs=fs, fa=fa)
    angle = (
        math.pi * 2.0
    )  # (360 * pi / 180) # ggrrr.. need to convert or debug for hours :-)
    bpy.ops.mesh.spin(
        steps=segments,
        dupli=False,
        angle=angle,
...
def round_edges(
    width=5.0, segments=4, verts_only=False, angle_limit=0.785398, o=None, apply=True
):
    if o is None:
        o = bpy.context.scene.objects.active
    else:
